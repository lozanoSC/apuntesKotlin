...
Mutable significa que a la variable se puede reasignar a un valor diferente después de la asignación inicial. Normalmente esto lo que queremos hacer con una variable, ir variando su valor a lo largo del programa.

Para declarar una variable mutable, usamos la palabra clave *var* como ya hizimos  en los ejemplos anteriores. En el siguiente ejemplo observamos que efectivamente una variable declarada con *var* puede cambiar de valor
~~~
fun main() {
    var nombre = "Juan Perro"
    println("Tu nombre: $nombre")
